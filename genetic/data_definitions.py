...
class Map:
    ...
    def __init__(self, origin, destination, areas_n, inflation_rate=0.1, mode="scalar"):
        self.origin = origin  # CartesianPoint: Route's origin point
        self.destination = destination  # CartesianPoint: Route's destination point
        self.areas_n = areas_n  # [area, ...]
        # area = [CartesianPoint(), ...]
        self.areas_n_inf = [
            self._inflate_area(area, inflation_rate=inflation_rate, mode=mode)
            for area in areas_n
        ]
    ...
    def _inflate_area(self, area, inflation_rate, mode):
        ...
        # TODO: estudar melhores metodos para realizar a inflação, lidar com problemas
